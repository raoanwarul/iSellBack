...
export async function requestNotificationPermission() {
  ...
  try {
    ...
    if (permission === 'granted') {
      ...
      console.log('✅ Agent Web FCM Token:', token);
      return token;
    }
...
export async function saveFcmTokenToSupabase(token) {
  try {
    ...
    console.log('✅ Agent web FCM token saved to Supabase');
  } catch (error) {
...
export async function deleteFcmToken() {
  try {
    ...
    await supabase.from('fcm_tokens')
      .delete()
      .eq('user_id', user.id)
      .eq('device_type', 'web_agent');
    console.log('✅ Agent web FCM token deleted');
  } catch (error) {
...
export function onForegroundMessage(callback) {
  ...
  onMessage(messaging, (payload) => {
    console.log('🔔 Agent Foreground message:', payload);
